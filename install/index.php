<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Whonix</title>
    <link rel="shortcut icon" href="img/favicon.png">

    <!-- Bootstrap core CSS -->
    <link href="/css/style.css" rel="stylesheet">
  </head>
  <body>


<div class="hero">
	<header>
  <div class="clearfix">
    <h1><a href="/" class="cc-active"><img width="160" src="/img/Whonix_Logo.png" alt="Whonix"></a></h1>

    <nav>
      <ul>
        <li><a href="/">Home</a></li>
        <li><a href="/download">Download</a></li>
        <li><a href="/intro" class="cc-active">FAQ</a></li>
		<li><a href="https://www.whonix.org/wiki/Main_Page">Wiki</a></li>
		<li><a href="/newsletter">Newsletter</a></li>
        <li><a href="https://forums.whonix.org/">Forum</a></li>
      </ul>
    </nav>
  </div>
</header>

	<div class="container">
		<h2 class="editable">Quick Start Guide</h2>
	</div>
</div>

<section class="alternate-section">
	<div class="container">
		<div class="left-column">
			<h3>Installation and Updating</h3>

			<div class="staff-profile">
				<h4 class="editable">Choose a Virtualization Platform</h4>
				<p>Insert Text</p>
			</div>

			<div class="staff-profile">
				<h4 class="editable">Download, verify</h4>
				<p>Insert Text</p>
			</div>

			<div class="staff-profile">
				<h4 class="editable">Install</h4>
				<p>Insert Text</p>
			</div>

			<div class="staff-profile">
				<h4 class="editable">Update</h4>
				<p>Insert Text</p>
			</div>

			<div class="staff-profile">
				<a href="/intro" class="button">< Back</a>
				<a href="/first" class="button">Next ></a>
			</div>

		</div>

		<div class="right-column">
			<h4>Chapters</h4>

			<ul>
				<p><a href="/intro">An Introduction to Whonix</a></p>
				<p><a href="/install">Installation and Updating</a></p>
				<p><a href="/first">First Steps in Whonix</a></p>
				<p><a href="/common">Common Tasks</a></p>
				<p><a href="/anonymity">Preserving your Privacy</a></p>
				<p><a href="/more">Where to go from here</a></p>
			</ul>
		</div>
	</div>
</section>

    <section>
		<div class="container subheader">
			<h3>Share Whonix with your friends:</h3>
			<p><a href="https://twitter.com/intent/tweet?url=https://www.whonix.org&text=&via=Whonix"><img src="/img/twitter.png" class="editable"></a><a> </a><a href="https://facebook.com/sharer.php?u=https://www.whonix.org"><img src="/img/facebook2.png" class="editable"></a><a> </a><a href="https://plus.google.com/share?url=https://www.whonix.org"><img src="/img/google-plus.png" class="editable"></a></p>
		</div>
	</section>

    <footer>
		<div class="container">
			<p><a href="https://www.whonix.org/wiki/Impressum">Impressum</a><a> </a><a href="https://www.whonix.org/wiki/Datenschutz">Datenschutz</a><a> </a><a href="https://www.whonix.org/wiki/Haftungsausschluss">Haftungsausschluss</a><a> </a><a href="https://www.whonix.org/wiki/Donate">Donate</a><a> </a><a href="https://www.whonix.org/wiki/Investors">Investors</a></p>
		</div>
    </footer>
  </body>
</html>
